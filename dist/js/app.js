(() => {
    ...
    function documentActions(e) {
        ...
        if (!target.classList.contains(".ancor") && scrol == true) body.addEventListener("click", stopAnimation);
    }
    let stop = false;
    let scrol = false;
    ...
    const scrolling = selectorBtn => {
        ...
        for (let i = 0; i < links.length; i++) links[i].addEventListener("click", (function(event) {
            event.preventDefault();
            let widthTop = Math.round(document.documentElement.scrollTop || document.body.scrollTop), hash = this.hash;
            ...
            function step(time) {
                ...
                let progress = time - start, r = toBlock < 0 ? Math.max(widthTop - progress / speed, widthTop + toBlock) : Math.min(widthTop + progress / speed, widthTop + toBlock);
                let element = document.documentElement || document.body;
                element.scrollTo(0, r);
                if (r != widthTop + toBlock && !stop) requestAnimationFrame(step); else {
    ...
    function marquee() {
        ...
        $marqueeArray.forEach(($wrapper => {
            ...
            const setBaseStyles = firstScreenVisibleSize => {
                let baseStyle = "display: flex; align-items: center; flex-wrap: nowrap;";
                if (isVertical) {
            ...
            sections.forEach(((section, i) => {
                if (section.offsetTop - document.querySelector("header").clientHeight - 30 <= scrollDistance) {
                    navLinks.forEach((link => {
                        if (link.classList.contains("_active-link")) link.classList.remove("_active-link");
                    }));
    ...
    function showHideHeader() {
        ...
        window.addEventListener("scroll", (function() {
            ...
            if (scrollTop > 1) {
                ...
                if (scrollTop > lastScrollTop) document.documentElement.classList.add("scrolled-down"); else document.documentElement.classList.remove("scrolled-down");
            } else document.documentElement.classList.remove("scrolled-page");
